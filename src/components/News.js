...
document.title=`${this.capitalizeFirstLetter(this.props.category)}-News-Hunt`;  
  ...
  render() {
    return (
      <>
        <h1 className="text-center my=3" style={{ margin: '35px 0px',marginTop:'90px',color:'white' }}>NewsHunt- Top {this.capitalizeFirstLetter(this.props.category)}</h1>
        ...
        <div className="row">
          {/*!this.state.loading && */}
           {this.state.articles.map((element)=>{
              ...
              <NewsItem title={element.title ? element.title : ""} description={element.description ? element.description.slice(0, 20) : ""} imageUrl={element.urlToImage} newsUrl={element.url} author={element.author} date={element.publishedAt} source={element.source.name} />
            </div>
          })}

          {/* {/* <div className="col-md-3">
            <NewsItem title="myTitle" description="mydesc" imageUrl="https://static.independent.co.uk/2022/07/05/11/SEI113124316.jpg?quality=75&width=1200&auto=webp" />
          </div> */}
